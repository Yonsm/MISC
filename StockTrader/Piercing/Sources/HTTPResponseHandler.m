...
+ (void)registerHandler:(Class)handlerClass
{
	...
	NSUInteger priority = [handlerClass priority];
	for (i = 0; i < count; i++)
	{
		NSUInteger pi = [registeredHandlers[i] priority];
		if (priority >= pi)
		{
			break;
		}
	...
	server:(HTTPServer *)aServer
{
	NSDictionary *requestHeaderFields =
		CFBridgingRelease(CFHTTPMessageCopyAllHeaderFields(aRequest));
	NSURL *requestURL =
		CFBridgingRelease(CFHTTPMessageCopyRequestURL(aRequest));
	NSString *method =
		CFBridgingRelease(CFHTTPMessageCopyRequestMethod(aRequest));
